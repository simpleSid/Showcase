...
$subject = 'Новая заявка ChildSquare';
$body = implode("\n", [
    'Имя: ' . $name,
    'Телефон: ' . $phone,
    '',
...
try {
    ...
    respond(200, ['message' => 'Заявка отправлена.']);
} catch (Throwable $error) {
    error_log('Contact form mail error: ' . $error->getMessage());
...
function sendSmtpMail(array $config, string $subject, string $body): void
{
    ...
    try {
        ...
        smtpCommand($socket, 'EHLO childsquare.local', [250]);
